...
def main():
    parser = argparse.ArgumentParser(description='Aggregate Apptainer OSU benchmark trials')
    parser.add_argument('--results-dir', default='osu_apptainer_results', help='Directory containing results')
    ...
    plt.ylabel(ylabel)
    plt.title(f'OSU Benchmark with Apptainer: {ylabel} across Configurations (Mean ± Min/Max)')
    plt.legend()
    plt.grid(True, which='both', ls='--', lw=0.5)
    plt.tight_layout()
    
    outfile = results_dir / f'apptainer_{args.out_prefix}_{args.type}_comparison.png'
    plt.savefig(outfile, dpi=150)
    print(f"Plot saved to {outfile}")
    
    # Save aggregated data to CSV
    csv_file = results_dir / f'apptainer_{args.out_prefix}_{args.type}_stats.csv'
    for label, agg in results.items():
        agg['config'] = label
